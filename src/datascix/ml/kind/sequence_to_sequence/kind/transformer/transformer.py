...
from datascix.ml.kind.sequence_to_sequence.kind.transformer.validation.kind.train_test import TrainTest
from datascix.ml.model.model import Model as BaseModel
...
class Transformer(TfModel, BaseModel):
    def __init__(
            self,
            model_dimension: int,
        ...
        self.output_projection = TfLayers.Dense(self.output_feature_count)

        # Learned positional embeddings (simplest for continuous features)
    ...
    def _decode(self, encoder_memory: tf.Tensor, decoder_input: tf.Tensor, training: bool) -> tf.Tensor:
        # decoder_input: (B, T_out, F_out)  (teacher forcing inputs)
        ...
        # Causal self-attention (decoder must not see future steps)
    ...
    def call(self, inputs, training: bool = False) -> tf.Tensor:
        ...
        encoder_input: (B, T_in, F_in)
        decoder_input: (B, T_out, F_out)  (shifted targets for teacher forcing)
        """
        encoder_input, decoder_input = inputs
        encoder_memory = self._encode(encoder_input, training=training)
        decoder_output = self._decode(encoder_memory, decoder_input, training=training)
        return decoder_output
    ...
    def predict_autoregressive(self, input_array: np.ndarray) -> np.ndarray:
        """
        Autoregressive inference:
        - Starts decoder input with zeros
        - Feeds previous predictions back into decoder input
        """
        ...
        decoder_sequence = tf.zeros((batch_size, 0, self.output_feature_count), dtype=tf.float32)
        ...
        while step < self.output_time_steps:
            if step == 0:
                ...
                current_decoder_input = tf.concat(
                    [tf.zeros((batch_size, 1, self.output_feature_count), dtype=tf.float32), decoder_sequence],
                    axis=1
                )
                current_decoder_input = current_decoder_input[:, :step + 1, :]

            decoder_output = self._decode(encoder_memory, current_decoder_input, training=False)
            next_step_prediction = decoder_output[:, -1:, :]
            decoder_sequence = tf.concat([decoder_sequence, next_step_prediction], axis=1)
            step += 1

        return decoder_sequence.numpy()
    ...
    def get_predictions(self, input_array: np.ndarray) -> np.ndarray:
        self.predict_autoregressive(input_array)

    def train(self, input_array, output_array, epochs, batch_size)->None:
        self.train(input_array, output_array, epochs, batch_size)

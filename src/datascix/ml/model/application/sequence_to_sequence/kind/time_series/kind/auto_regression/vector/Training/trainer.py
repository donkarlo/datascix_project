...
from datascix.ml.model.application.sequence_to_sequence.kind.time_series.kind.auto_regression.vector.Training.config import \
    Config
from datascix.ml.model.application.sequence_to_sequence.kind.time_series.kind.auto_regression.vector.Training.learned_parameters import \
    LearnedParameters
from datascix.ml.model.application.sequence_to_sequence.kind.time_series.training.training import \
    ...
    def get_learned_parameters(self) -> LearnedParameters:
        if self._learned_parameters is None:
            ...
            if maximum_lag_order <= 0:
                raise ValueError("architecture.lag_order must be > 0.")

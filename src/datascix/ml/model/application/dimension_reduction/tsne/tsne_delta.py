from utilix.data.storage.kind.file.pkl.pkl import Pkl
from utilix.os.file_system.file.file import File
from utilix.os.file_system.path.path import Path

import numpy as np
from sklearn.manifold import TSNE
import matplotlib.pyplot as plt


class TsneDelta:
    """
    Computes a 2D t-SNE embedding of LiDAR scan deltas
    to separate straight motion from corner motion.
    """

    def __init__(self) -> None:
        path = Path(
            "/pair_set/experiements/oldest/robots/uav1/mind/memory/long_term/explicit/episodic/normal/lidar_scan_ranges_sliced_from_1_to_300000/lidar_scan_ranges_sliced_from_1_to_300000.pkl"
        )

        os_file = File.init_from_path(path)
        pickle = Pkl(os_file, False)
        scan_sliced_values = pickle.load()
        scan_values = scan_sliced_values.get_values()

        lidar_vectors = []
        for scan_value in scan_values:
            lidar_vectors.append(
                scan_value
                .get_formatted_data()
                .get_vector_representation()
                .get_components()
            )

        lidar_vectors = np.array(lidar_vectors, dtype=np.float64)

        # ----- Clean LiDAR pair_set -----
        max_range = 15.0
        lidar_vectors[~np.isfinite(lidar_vectors)] = max_range
        lidar_vectors[lidar_vectors > max_range] = max_range
        lidar_vectors[lidar_vectors < 0.0] = 0.0

        print("Original lidar shape:", lidar_vectors.shape)

        # ----- Build delta vectors: scan(t) - scan(t-1) -----
        # This emphasizes changes: corners vs straight motion
        delta_vectors = lidar_vectors[1:] - lidar_vectors[:-1]
        print("Delta shape:", delta_vectors.shape)

        # Optionally clip deltas to reduce effect of outliers
        delta_max = 5.0
        delta_vectors[delta_vectors > delta_max] = delta_max
        delta_vectors[delta_vectors < -delta_max] = -delta_max

        # ----- Downsample for faster t-SNE -----
        target_sample_count = 10000
        if delta_vectors.shape[0] > target_sample_count:
            step = delta_vectors.shape[0] // target_sample_count
            if step < 1:
                step = 1
            delta_vectors = delta_vectors[::step]

        print("Shape  used for t-SNE:", delta_vectors.shape)

        # ----- t-SNE to 2 dimensions -----
        tsne = TSNE(
            n_components=2,
            perplexity=30.0,
            learning_rate=200.0,
            init="random",
            random_state=42,
            verbose=1,
        )

        embedding = tsne.fit_transform(delta_vectors)

        print("t-SNE embedding shape:", embedding.shape)

        # ----- Simple 2D scatter plot -----
        plt.figure()
        plt.scatter(embedding[:, 0], embedding[:, 1], s=2)
        plt.xlabel("t-SNE 1")
        plt.ylabel("t-SNE 2")
        plt.title("t-SNE embedding of LiDAR scan deltas")
        plt.tight_layout()
        plt.show()


if __name__ == "__main__":
    tsne_delta = TsneDelta()
